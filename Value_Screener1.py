    ...
    if uploaded_file is not None:
        try:
            ...
            if missing_cols:
                ...
                st.warning("Please upload a file with all required columns to proceed.")
                return None
...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_file = st.file_uploader(
            "Upload Excel File", 
            type=['xlsx', 'xls'],
            help="Excel file should contain: Symbol, Exchange, Sector, Industry, Theme, Name, Country, Notes, Asset_Type"
        )
    ...
    # Load data only if file is uploaded and valid
    if st.session_state.get('file_uploaded', False):
        ...
        if df is not None:
            ...
            if not filtered_df.empty:
                with st.spinner("Fetching financial data... This may take a moment for large datasets."):
                    progress_bar = st.progress(0)
                    metrics_data = []
                    total_symbols = len(filtered_df['Symbol'].unique())
                    ...
                    if metrics_data:
                        ...
                        if not value_filtered.empty:
                            value_filtered = value_filtered[
                                (value_filtered['P/E'] <= pe_max) & 
                                (value_filtered['P/B'] <= pb_max) & 
                                (value_filtered['ROE'] >= roe_min/100)
                ...
                st.warning("No companies match your initial filters. Try adjusting your sector/country selections.")
    
    elif uploaded_file is None and not st.session_state.file_uploaded:
