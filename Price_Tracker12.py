...
def fetch_current_week_close(symbol, current_week_start):
    ...
    st.write(f"{symbol} current week data:")
    st.dataframe(data)
    if data.empty or "Close" not in data.columns:
        return np.nan
    # Take the latest available close (even if not today)
...
if uploaded_file:
    ...
    if not all(col in tickers_df.columns for col in ["Symbol", "Exchange"]):
        ...
        for symbol in symbols:
            ...
            if closes is not None and len(closes) == 6 and not np.isnan(current_close):
                result[symbol] = closes + [current_close]
            else:
                st.warning(f"Ticker {symbol}: Could not fetch 6 weeks of valid closing prices and a current close. Skipped.")
        if result:
            ...
            tab1, tab2, tab3 = st.tabs(["Price Trend", "Normalized Performance", "Ticker Scores (5 Strategies)"])
