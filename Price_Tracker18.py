    ...
    if sheet_choice and sheet_choice in sheet_names:
        ...
        if not all(col in tickers_df.columns for col in ["Symbol", "Exchange"]):
            ...
            if result:
                        ...
                        for sym in tickers_to_plot:
                            ...
                            if not row.empty:
                                fig.add_trace(go.Scatter(
                                    x=non_nan_cols,
                                    y=row[non_nan_cols].iloc[0],
                                    mode="lines+markers",
                                    name=sym
                                ))
                        fig.update_layout(
                            title="Weekly Closing Price Trend",
                            xaxis_title="Week (Friday to Friday, Current: Fri to latest close)",
                ...
                # ---- NORMALIZED PERFORMANCE TAB with Plotly ----
                with tab2:
                    ...
                    if tickers_to_plot:
                        ...
                        fig.update_layout(
                            title="Normalized Weekly Performance",
                            xaxis_title="Week",
